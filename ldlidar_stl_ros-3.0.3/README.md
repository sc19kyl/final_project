...
将下载的lidar_stl_ros-x.x.x.zip ROS 包复制到home目录，或你能找到的目录。

```bash
cd ~

unzip lidar_stl_ros-x.x.x

mkdir -p ldlidar_ros_ws/src

cd ldlidar_ros_ws/src

cp ../../lidar_stl_ros-x.x.x/* ./ -r
...
- 产品型号为 youyeetoo LiDAR FHL-LD06
  - 启动ld06 lidar node:
  ``` bash
...
- 产品型号为 youyeetoo LiDAR FHL-LD19
  - 启动ld19 lidar node:
  ``` bash
...
> This SDK is only applicable to the product models are :
> - youyeetoo LiDAR FHL-LD06
> - youyeetoo LiDAR FHL-LD19
## step0: get LiDAR ROS Package

Copy the downloaded lidar_stl_ros-x.x.x.zip ROS package to the home directory, or to a directory you can find.
...
- The product is youyeetoo LiDAR FHL-LD06
...
- The product is youyeetoo LiDAR FHL-LD19
